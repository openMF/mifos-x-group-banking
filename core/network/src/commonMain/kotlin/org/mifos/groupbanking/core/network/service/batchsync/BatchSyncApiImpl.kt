 ...
 */
package org.mifos.groupbanking.core.network.service.batchsync
...
private const val TAG = "BatchSyncApi"
private const val BATCHES_PATH = "/fineract-provider/api/v1/batches"

/**
 * Plain-Ktor implementation of [BatchSyncApi]. This class is the ONLY layer in the sync-status
 ...
 * (Mandatory Rule 4 forbids try-catch one layer up, in `SyncManagerImpl`). The HTTP-status ->
 ...
 * [ContentConvertException] on the response-decode path — covers a malformed 2xx body (including
 * a non-array JSON body for this array-shaped response contract).
 *
 * Reuses the shared `HttpClient` singleton registered in `kpt.core.network.di.NetworkModule` — no
 * second engine constructed here, matching every other Service in this module.
 *
 * See API.md#services — BatchSyncApi.
 */
class BatchSyncApiImpl(
    ...
    override suspend fun batchSync(
        ...
        Logger.d(TAG) { "batchSync: POST $BATCHES_PATH (requests=${request.requests.size})" }
        return requestAsNetworkResult(op = "batchSync") {
            httpClient.post(BATCHES_PATH) {
 ...
 * [TAG]/[op]. `reified T` here is instantiated as `List<BatchSyncResponseItemDto>` — Ktor's
 * content-negotiation decodes the top-level JSON array directly into it, no manual
 * `ListSerializer` wiring needed at the call site.
 */
private suspend inline fun <reified T> requestAsNetworkResult(
    ...
    return if (response.status.value in 200..299) {
        try {
            ...
            // "not-json", or a non-array JSON object for this array-shaped contract).
            Logger.e(TAG) { "$op response deserialization failure: ${e.message}" }
            NetworkResult.Error(NetworkError.SERIALIZATION)
        }
    } else {
