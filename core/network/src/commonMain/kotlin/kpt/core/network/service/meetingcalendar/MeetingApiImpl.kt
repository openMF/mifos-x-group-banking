 ...
 */
package kpt.core.network.service.meetingcalendar
...
private const val TAG = "MeetingApi"
private const val FINERACT_BASE = "/fineract-provider/api/v1"

/**
 * Plain-Ktor implementation of [MeetingApi]. This class is the ONLY layer in the meeting-calendar
 ...
 * (Mandatory Rule 4 forbids try-catch one layer up, in the Store5 wrapper / Repository). The
 * HTTP-status -> [NetworkError] mapping mirrors `core-base/network`'s
 * `ResultSuspendConverterFactory` table exactly, same as
 * [kpt.core.network.service.groupdashboard.GroupDashboardApiImpl].
 *
 * Reuses the shared `HttpClient` singleton registered in `kpt.core.network.di.NetworkModule` — no
 * second engine constructed here.
 *
 * See API.md#services — MeetingApi.
 */
