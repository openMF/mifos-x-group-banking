 ...
 */
package kpt.core.network.service.loanrequest
...
import kpt.core.network.model.ClientAccountsDto
import kpt.core.network.model.LoanRequestPayloadDto
import kpt.core.network.model.LoanRequestResponseDto

private const val TAG = "LoanRequestApi"
private const val LOAN_REQUEST_DATATABLE_PATH = "/datatables/dt_loan_request"

/**
 * Plain-Ktor implementation of [LoanRequestApi]. This class is the ONLY layer in the loan-request
 * client stack allowed a try-catch — every exception (transport failure, serialization failure) is
 * caught HERE and converted into [NetworkResult.Error]; nothing throws past this boundary
 * (Mandatory Rule 4 forbids try-catch one layer up, in `LoanRequestRepositoryImpl`). The
 * HTTP-status -> [NetworkError] mapping mirrors `core-base/network`'s `ResultSuspendConverterFactory`
 * table exactly, same as
 * [kpt.core.network.service.loanapply.LoanApplyApiImpl] /
 * [kpt.core.network.service.memberadd.MemberAddApiImpl]. Also catches
 * [ContentConvertException] on the response-decode path — the malformed-2xx-body gap
 * [kpt.core.network.service.loanapply.LoanApplyApiImpl] first flagged as
 * missing from the shared loanlist/memberadd precedent's `requestAsNetworkResult` helper.
 *
 * Reuses the shared `HttpClient` singleton registered in `kpt.core.network.di.NetworkModule` — no
 * second engine constructed here, even though this path is a raw Fineract datatable passthrough
 * (`/datatables/dt_loan_request`) rather than a `/companion/…` one; both are served by the same
 * host today, same convention as
 * [kpt.core.network.service.memberadd.MemberAddApiImpl].
 *
 * See API.md#services — LoanRequestApi.
 */
class LoanRequestApiImpl(
    ...
    override suspend fun submitLoanRequest(
        ...
        Logger.d(TAG) {
            "submitLoanRequest: POST $LOAN_REQUEST_DATATABLE_PATH (clientId=${request.clientId}, " +
                "requestedAmount=${request.requestedAmount})"
        }
        return requestAsNetworkResult(op = "submitLoanRequest") {
            httpClient.post(LOAN_REQUEST_DATATABLE_PATH) {
...
private suspend inline fun <reified T> requestAsNetworkResult(
    ...
    return if (response.status.value in 200..299) {
        try {
            ...
            // "not-json") — see class KDoc for the loan-apply precedent this closes.
            Logger.e(TAG) { "$op response deserialization failure: ${e.message}" }
            NetworkResult.Error(NetworkError.SERIALIZATION)
        }
    } else {
 ...
 * (400/401/404/408/429/5xx/else->UNKNOWN) — an unmapped code (e.g. 409 Conflict, which
 * [NetworkError] has no dedicated bucket for) intentionally falls into [NetworkError.UNKNOWN], same
 * as that converter's `else` branch.
 */
