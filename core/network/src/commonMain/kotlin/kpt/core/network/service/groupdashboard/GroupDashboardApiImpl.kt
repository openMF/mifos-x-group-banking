 ...
 */
package kpt.core.network.service.groupdashboard
...
import kpt.core.network.model.GroupAccountsDto
import kpt.core.network.model.GroupCorpusDto
import kpt.core.network.model.GroupDetailDto
import kpt.core.network.model.ViewerRoleInfoDto

private const val TAG = "GroupDashboardApi"
private const val GROUPS_PATH = "/companion/groups"

/**
 * Plain-Ktor implementation of [GroupDashboardApi]. This class is the ONLY layer in the
 * group-dashboard client stack allowed a try-catch — every exception (transport failure,
 ...
 * Repository / Store5 wrapper). The HTTP-status -> [NetworkError] mapping mirrors
 ...
 * [kpt.core.network.service.personaldashboard.MemberDashboardApiImpl].
 *
 ...
 * a per-call header — no `sessionToken` parameter is threaded through any method here.
 *
 * See API.md#services — GroupDashboardApi.
 */
class GroupDashboardApiImpl(
    private val httpClient: HttpClient,
) : GroupDashboardApi {

    override suspend fun getGroup(groupId: String): NetworkResult<GroupDetailDto, NetworkError> {
        val path = "$GROUPS_PATH/$groupId"
        Logger.d(TAG) { "getGroup: GET $path" }
        return requestAsNetworkResult(op = "getGroup") {
            httpClient.get(path)
        }
    }

    override suspend fun getViewerRole(groupId: String): NetworkResult<ViewerRoleInfoDto, NetworkError> {
        val path = "$GROUPS_PATH/$groupId/my-role"
        Logger.d(TAG) { "getViewerRole: GET $path" }
        return requestAsNetworkResult(op = "getViewerRole") {
            httpClient.get(path)
        }
    }

    override suspend fun getGroupCorpus(groupId: String): NetworkResult<GroupCorpusDto, NetworkError> {
        val path = "$GROUPS_PATH/$groupId/corpus"
    ...
    override suspend fun getGroupAccounts(groupId: String): NetworkResult<GroupAccountsDto, NetworkError> {
        val path = "$GROUPS_PATH/$groupId/accounts"
        Logger.d(TAG) { "getGroupAccounts: GET $path" }
        return requestAsNetworkResult(op = "getGroupAccounts") {
            httpClient.get(path)
        }
    }
}
