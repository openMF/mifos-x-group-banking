 ...
 */
package kpt.core.network.service.grouplist
...
import kpt.core.network.model.GroupPageDto

private const val TAG = "GroupApi"
private const val MY_GROUPS_PATH = "/companion/groups/mine"

/**
 * Plain-Ktor implementation of [GroupApi]. This class is the ONLY layer in the group-list client
 * stack allowed a try-catch — every exception (transport failure, serialization failure) is
 ...
 * [kpt.core.network.service.grouptypepicker.GroupTypeConfigApiImpl] and
 * [kpt.core.network.service.loginsignup.CompanionAuthApiImpl].
 *
 ...
 * a per-call header — no `sessionToken` parameter is threaded through [GroupApi.getMyGroups].
 *
 * See API.md#services — GroupApi.
 */
class GroupApiImpl(
    private val httpClient: HttpClient,
) : GroupApi {

    override suspend fun getMyGroups(paged: Boolean, limit: Int, offset: Int): NetworkResult<GroupPageDto, NetworkError> {
        Logger.d(TAG) { "getMyGroups: GET $MY_GROUPS_PATH (paged=$paged, limit=$limit, offset=$offset)" }
        return requestAsNetworkResult(op = "getMyGroups") {
            httpClient.get(MY_GROUPS_PATH) {
