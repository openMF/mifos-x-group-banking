 ...
 */
package kpt.core.model.meeting
 ...
 *
 * Namespaced under `kpt.core.model.meeting` deliberately: the meeting projection
 * of a group member ([GroupMember] here — `memberId`/`initials`/`role`) is a DISTINCT shape from the
 * existing `kpt.core.model.GroupMember` (`fineractClientId`/`displayName`), and
 * likewise [LoanSummary] here differs from `kpt.core.model.LoanSummary`. Keeping
 ...
 * [kpt.core.data.repository.MeetingConductRepository.loadMeetingData] and applied
