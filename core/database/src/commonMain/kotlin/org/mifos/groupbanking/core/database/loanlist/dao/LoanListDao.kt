 ...
 */
package org.mifos.groupbanking.core.database.loanlist.dao
...
/**
 * Room DAO for the paginated loan-list cache (SourceOfTruth for the PAGINATED
 * NETWORK_WITH_CACHE `LoansPagingStore`).
 *
 ...
 * [LoanListEntity.groupId] + [LoanListEntity.pageIndex]. [replacePage] is the store's
 ...
 */
@Dao
interface LoanListDao {

    /** Reactive read of one cached page of a group in server order. Backs the store SoT reader. */
    @Query(
        "SELECT * FROM loan_list_cache WHERE groupId = :groupId AND pageIndex = :pageIndex " +
            "ORDER BY rowOrder ASC",
    )
    fun observePage(groupId: String, pageIndex: Int): Flow<List<LoanListEntity>>

    /** Reactive read of one group's whole cache in (page, row) order — diagnostics / whole-list reads. */
    @Query(
        "SELECT * FROM loan_list_cache WHERE groupId = :groupId " +
            "ORDER BY pageIndex ASC, rowOrder ASC",
    )
    fun observeGroup(groupId: String): Flow<List<LoanListEntity>>

    @Upsert
    suspend fun upsertAll(entities: List<LoanListEntity>)

    @Query("DELETE FROM loan_list_cache WHERE groupId = :groupId AND pageIndex = :pageIndex")
    suspend fun deletePage(groupId: String, pageIndex: Int)

    @Query("DELETE FROM loan_list_cache WHERE groupId = :groupId")
    suspend fun deleteGroup(groupId: String)

    @Query("DELETE FROM loan_list_cache")
     ...
     */
    @Transaction
    suspend fun replacePage(groupId: String, pageIndex: Int, entities: List<LoanListEntity>) {
        deletePage(groupId, pageIndex)
        upsertAll(entities)
    }

    /** TTL pruning hook — removes rows written before [epochMillis]. */
    @Query("DELETE FROM loan_list_cache WHERE fetchedAt < :epochMillis")
