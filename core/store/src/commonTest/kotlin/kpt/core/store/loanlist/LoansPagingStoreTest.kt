 ...
 */
package kpt.core.store.loanlist
...
import kpt.core.network.model.LoanPageDto
import kpt.core.network.model.LoanSummaryDto
import kpt.core.network.service.loanlist.LoanApi
import kpt.core.store.loanlist.impl.LoanListFetchException
...
/**
 * TDD coverage for [provideLoansPagingStore] — the PAGINATED NETWORK_WITH_CACHE loan-list store
 * (offset-paginated loan accounts belonging to a savings group, `GET /groups/{groupId}/loans`).
 *
 * Exercises the Store5 paging read pipeline directly (per-page Fetcher + Room-shaped
 * SourceOfTruth + validator) with an in-memory [FakeLoanListDao] and a scripted [FakeLoanApi];
 ...
 *
 * Key = [PageKey] with the groupId carried in [PageKey.query] as its String form (offset paging,
 * `pageSize=20`) so the cache is per-group AND per-page; the store re-parses it to a `Long` for the
 * Fineract fetcher. Value = `List<LoanSummary>` (one page slice).
 */
class LoansPagingStoreTest {
    ...
    fun first_page_cache_miss_calls_fetcher_persists_to_sot_and_emits() = runTest {
        val api = FakeLoanApi(pages = mapOf(key(GROUP_A, 0) to page(0, 2)))
        val dao = FakeLoanListDao()
        val store = provideLoansPagingStore(api, dao)
        ...
        assertEquals(2, dao.rowsForPage(GROUP_A.toString(), 0).size, "fetched page must be written through to the SoT")
    }
    ...
    fun load_more_fetches_next_page_at_offset() = runTest {
        ...
        assertEquals(20, dao.rowsForPage(GROUP_A.toString(), 0).size, "page 0 rows are retained")
        assertEquals(5, dao.rowsForPage(GROUP_A.toString(), 1).size, "page 1 rows persisted under their own pageIndex")
    }
    ...
    fun cache_hit_emits_from_sot_without_fetcher_call() = runTest {
        val api = FakeLoanApi(pages = mapOf(key(GROUP_A, 0) to page(0, 2)))
        val dao = FakeLoanListDao().apply { seedPage(GROUP_A.toString(), 0, entities(GROUP_A.toString(), 0, 2)) }
        val store = provideLoansPagingStore(api, dao)
    ...
    fun fetcher_error_surfaces_as_error_response() = runTest {
        ...
        assertTrue(error is LoanListFetchException, "error channel must carry the typed fetch exception")
    ...
    fun page_refetch_replaces_prior_rows_atomically_no_staleness() = runTest {
        val api = FakeLoanApi(pages = mapOf(key(GROUP_A, 0) to page(0, 5)))
        val dao = FakeLoanListDao().apply { seedPage(GROUP_A.toString(), 0, entities(GROUP_A.toString(), 0, 20)) } // 20 stale rows
        val store = provideLoansPagingStore(api, dao)

        store.loadPage(pageKey(GROUP_A, 0), refresh = true)

        assertEquals(5, dao.rowsForPage(GROUP_A.toString(), 0).size, "replacePage must drop the 20 stale rows, keep only the 5 fresh")
    ...
    fun distinct_groups_cache_pages_independently() = runTest {
        ...
        assertEquals(3, dao.rowsForPage(GROUP_A.toString(), 0).size, "group A page 0 cached under its own groupId")
        assertEquals(7, dao.rowsForPage(GROUP_B.toString(), 0).size, "group B page 0 cached under its own groupId")
    }

    private companion object {
        const val PAGE_SIZE = 20
        const val GROUP_A = 101L
        const val GROUP_B = 202L

        fun pageKey(groupId: Long, page: Int): PageKey =
            PageKey(page = page, pageSize = PAGE_SIZE, query = groupId.toString())
...
private class FakeLoanListDao : LoanListDao {
    ...
    fun rowsForPage(groupId: String, pageIndex: Int): List<LoanListEntity> =
